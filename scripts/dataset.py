...
class DataMatrixDataset(Dataset):
    def __init__(
        ...
        for folder_1 in os.listdir(self.root_dir):
            if os.path.isdir(os.path.join(root_dir, folder_1)):
                for folder_2 in os.listdir(os.path.join(root_dir, folder_1)):
                    for image_name in os.listdir(os.path.join(root_dir, folder_1, folder_2)):
                        image_path = os.path.join(root_dir, folder_1, folder_2, image_name)
                        if image_path.lower().endswith(("png", "jpg", "jpeg", "bmp", "gif")):
                            self.image_paths.append(image_path)
                            self.labels.append(f"{folder_1}/{folder_2}")
